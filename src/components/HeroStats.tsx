...
const HeroStats = () => {
    const containerRef = useAnimeStaggerReveal<HTMLDivElement>('.stat-card', {
        delay: 600,
        staggerDelay: 100,
        translateY: 20
    });
    ...
    return (
        <div ref={containerRef} className="w-full max-w-7xl mx-auto px-4 mt-10 md:mt-8">
            <div className="bg-white rounded-3xl xl:rounded-full shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-gray-100 p-3 md:py-4 md:px-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-1 lg:gap-3 bg-white">
                    {stats.map((stat, index) => (
                        <div key={index} className="stat-card flex items-center gap-1 justify-start md:justify-center relative group">
