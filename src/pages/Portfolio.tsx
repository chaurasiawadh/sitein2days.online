...
export default function Portfolio() {
    ...
    const filteredStudies = filter === 'All Projects'
        ? allProjects.filter(study => !study.isEcommerce)
            ...
            >
                <AnimatePresence mode='popLayout'>
                    {filteredStudies.map((study) => (
                        <Link
                            to={study.isEcommerce ? "#" : `/portfolio/${study.slug}`}
                            key={study.id}
                            className="block h-full"
                            onClick={study.isEcommerce ? (e) => e.preventDefault() : undefined}
                        >
                            <motion.div
                                layout
                                ...
                                className={`group h-full flex flex-col transition-all duration-300 ${study.isEcommerce
                                    ? 'rounded-3xl overflow-hidden border border-gray-200/50 shadow-sm hover:shadow-lg -translate-y-0 hover:-translate-y-2 bg-white'
                            ...
                            >
                                ...
                                <div className="p-5 flex flex-col flex-grow">
                                    <div className="mb-3">
                                        <span className="text-[10px] font-bold uppercase tracking-wider px-4 py-2 rounded-full inline-block text-gray-500 border border-gray-200">
                                            {study.industry}
                                        </span>
                                    </div>
                                    <h2 className="text-xl font-black mb-2 transition-all duration-300 text-black group-hover:text-purple-600">
                                        {study.client}
                                    </h2>
