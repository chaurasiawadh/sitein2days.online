...
function ServiceCard({ service, idPrefix }: { service: ServiceItem; idPrefix: string }) {
    return (
        <div id={service.title.toLowerCase().replace(/\s+/g, '-')} className="grid lg:grid-cols-2 gap-12 items-start scroll-mt-32 mb-10 sm:mb-20">
                    ...
                    <div className="mt-10">
                        ...
                        <ul className="space-y-2">
                            {service.growthPhilosophy.points.map((p, i) => (
                                <li key={i} className="text-sm text-gray-600 leading-relaxed">{p}</li>
                            ))}
                        </ul>
                        {service.timeline && (
...
export default function ServicesPage() {
    useEffect(() => {
        document.title = 'Our Services | sitein2days.online';
    }, []);
        ...
        <div className="min-h-screen pt-16 pb-16 sm:pt-24 sm:pb-20">
            ...
            <div className="px-4 max-w-7xl mx-auto space-y-16 sm:space-y-32">
                {CORE_SERVICES.map((service, index) => (
                    <ServiceCard key={index} service={service} idPrefix="core" />
                ))}
                ...
                {DATA_SERVICES.map((service, index) => (
                    <ServiceCard key={`data-${index}`} service={service} idPrefix="data" />
                ))}
            </div>
